...
const useBICallRequests = (from: number, to: number): Output => {
  const { data: selectedBusiness } = useBusiness();

  const { data, isLoading } = useQuery<Record<string, number>, Error>(
    [ANALYTICS_VIEW_QUERY_KEY, selectedBusiness?.slug, from, to, 'cr'],
    async () => AnalyticsServices.getCallRequests(selectedBusiness?.slug || '', { from, to }),
    {
...
export default useBICallRequests;
