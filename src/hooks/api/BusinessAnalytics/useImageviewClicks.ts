...
const useBIImageviewClicks = (from: number, to: number): Output => {
  const { data: selectedBusiness } = useBusiness();

  const { data, isLoading } = useQuery<Record<string, number>, Error>(
    [ANALYTICS_VIEW_QUERY_KEY, selectedBusiness?.slug, from, to, 'iv'],
    async () => AnalyticsServices.getImageClicks(selectedBusiness?.slug || '', { from, to }),
    {
...
export default useBIImageviewClicks;
