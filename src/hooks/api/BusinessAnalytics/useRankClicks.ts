...
const useBIRankClicks = (from: number, to: number): Output => {
  const { data: selectedBusiness } = useBusiness();

  const { data, isLoading } = useQuery<Record<string, number>, Error>(
    [ANALYTICS_VIEW_QUERY_KEY, selectedBusiness?.slug, from, to, 'rc'],
    async () => AnalyticsServices.getRankPageClicks(selectedBusiness?.slug || '', { from, to }),
    {
...
export default useBIRankClicks;
